...
- [ ] エディタリッチテキスト領域の修正
  - [ ] 折りたためるように
  - [ ] shadcnのcollapsedコンポーネント追加
  - [ ] 動画のサポート
  - [ ] URLのサポート
  - [ ] URLはOGPを取得、og:image, og:title, og:descriptionのカードを挿入できる形にしたい
  - [ ] ついでにremotion側で簡易的にリッチエディタの内容表示
- [ ] ホットキー追加
  - [ ] TTS削除（選択中のもの）
  - [ ] ページ追加（追加ダイアログ呼び出し）
- [ ] Voicevoxサポート
